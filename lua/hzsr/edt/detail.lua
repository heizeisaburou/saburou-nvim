...
---@class hzsr.edt.detail.reveal_opts
...
---
---@param bufnr integer
---@param opts hzsr.edt.detail.reveal_opts
...
---
---@param status hzsr.io.cst.status
---@param bufnr integer
---@param path? string
---@param msg? string
---@param extra? table
...
function M.add_batch_result(report, index, out)
  ...
  if out.status == hzsr.io.cst.status.SUCCESS then
  ...
  if out.status == hzsr.io.cst.status.REJECT then
    table.insert(report.rejected, out.bufnr)
    return
  end

  if out.status == hzsr.io.cst.status.CANCEL then
...
function M.make_batch_unrun_report(status, bufnrs, make_out)
  ...
  if status == hzsr.io.cst.status.CANCEL and #bufnrs > 0 then
    report.cancelled = 1
  end
  ...
  for _, bufnr in ipairs(bufnrs) do
    ...
    if status == hzsr.io.cst.status.REJECT then
      table.insert(report.rejected, bufnr)
    elseif status ~= hzsr.io.cst.status.CANCEL then
