...
function M.kill_opencode()
  ...
  if ok and state.opencode_server then
...
--  NOTE:
-- Copiado/adaptado de opencode.nvim `cycle_variant`.
-- La acción original cicla: nil -> low -> medium -> high -> nil.
-- Nosotros queremos navegación saturada:
--   <C-.> sube hasta high y se queda ahí
--   <C-,> baja hasta low y se queda ahí
-- Si el plugin cambia internamente `cycle_variant`, revisar esta función.
local function set_variant_delta(delta)
  local state = require "opencode.state"
  ...
  local current_index = util.index_of(variants, state.current_variant)

  if not current_index then
    current_index = delta > 0 and 0 or #variants + 1
  end

  local next_index = current_index + delta
  next_index = math.max(1, math.min(#variants, next_index))
...
local function variant_up()
  set_variant_delta(1)
end

local function variant_down()
  set_variant_delta(-1)
end
...
local function setup_custom_callback_keymaps()
  ...
  map({ "n", "i" }, "<A-o>", toggle_opencode, { desc = "Opencode: Toggle" })
