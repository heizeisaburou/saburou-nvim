...
compilar los parsers que usa la configuración: no compila él mismo, ejecuta `tree-sitter build`, y
es esa orden la que llama al [Compilador de C](/docs/Compilador%20de%20C.md).
...
> [!NOTE]
>
> La opción `--locked` obliga a Cargo a utilizar las versiones de las dependencias especificadas en
> `Cargo.lock`. Si no puede hacerlo, la instalación falla en lugar de resolver versiones
> diferentes.
