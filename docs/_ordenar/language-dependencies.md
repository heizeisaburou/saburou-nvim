...
La matriz vive en el [README](/README.md#lenguajes-soportados) y solo ahí: lenguaje, filetype, LSP,
formatter, linter y parser de Tree-sitter, con los nombres exactos que hay que descomentar en cada
lista. Tenerla duplicada aquí era la razón por la que se quedaban lenguajes fuera —se añadía el
soporte, se actualizaba una copia y la otra envejecía—, así que esta guía se queda con lo que la
tabla no puede contar: qué hay que instalar en el sistema, cómo, y qué se rompe si falta.
...
`shuck` es el language server, y es el único nombre de `M.servers` que no se parece en nada a su
lenguaje: por eso zsh estuvo soportado durante meses sin aparecer en la matriz. El mapeo de Mason ya
está en [names.lua](/lua/hzsr/mason/nvchad/names.lua) (`shuck = "shuck"`), así que basta con
descomentarlo.

Da diagnósticos con código propio (`C003` y compañía) y entiende directivas en comentario para
seguir los `source` que hace tu configuración:
...
Sin `lint=true` importa los símbolos —navegación y fin de los `C003`— pero no analiza el contenido
del archivo importado. Merece la pena en tus propios archivos; no en Powerlevel10k, FZF o cualquier
cosa de `/usr/share`, donde solo consigues diagnósticos sobre código que no controlas. Los detalles,
en [shuck.md](/docs/_ordenar/shuck.md).

**No hay formatter.** `shfmt` es lo que habría, y formatea POSIX/bash/mksh, pero no zsh: la sintaxis
propia de zsh (`[[ ]]` con sus operadores, expansiones y globs) no está en su gramática y el
resultado no es fiable. Está anotado en la cabecera de
[conform.lua](/lua/lzy/conform.lua) para que no se vuelva a intentar.

El parser de Tree-sitter es `zsh` y la detección de filetype viene de serie en Neovim (`.zsh`,
`.zshrc`, `.zshenv` y compañía): no hay que añadir nada a `opts.lua`.
...
y esa decisión no depende de la matriz.
