...
suelto **corta el destino** y el enlace se rompe en GitHub.
...
`Mi nota larga.md`, `[[mi-nota]]` no existe aunque su título diga otra cosa: la identidad de una nota
...
La **copia inteligente** copia lo que haya bajo el cursor sin sus delimitadores: el contenido de un
bloque de código, el destino o la etiqueta de un enlace, el texto en negrita... Y sobre un
...
`:MarksmanRelink` es útil después de cambiar el estilo en `.marksman.toml`: te dice cuántos enlaces
va a tocar y en qué estilo, y espera tu confirmación. Nunca toca un enlace externo, ni uno que no
resuelva, ni uno ambiguo.

## Crear una nota

Escribe un enlace a una nota que no existe y pulsa `<CR>`. Te pregunta el nombre —con el del enlace
ya puesto, así que basta con aceptar— y crea el archivo. Si cambias el nombre, el enlace se
reapunta solo.

## Avisos

Se marcan los enlaces que no llevan a ninguna parte y los ambiguos (más de una nota responde a
ese nombre). Dentro de un bloque de código no se avisa de nada: ahí un `[[ejemplo]]` es texto, no
un enlace.
