...
Una nota se llama **exactamente como la escribiste**: "Mi Nota Chula" crea `Mi Nota Chula.md`, con sus
espacios y sus mayúsculas. Ni ids generados ni guiones.

Vale tanto si la creas con `<leader>nn` como si sigues un `[[Mi Nota Chula]]` que todavía no existe.
Si el nombre ya está ocupado, se añade un número: `Mi Nota Chula 2`.
...
suelto **corta el destino** y el enlace se rompe en GitHub. Si prefieres verlo legible, los ángulos
también valen: `[texto](</docs/Mi nota.md>)`.
...
Los adjuntos también: `[[imagen.png]]` enlaza y `![[imagen.png]]` incrusta. El `!` significa incrustar,
no "adjunto".
...
bloque de código, el destino o la etiqueta de un enlace, el texto en negrita... Sobre un heading
...
Al revés no ocurre solo: si borras la nota que colisionaba, los enlaces se quedan largos hasta
que ejecutes `:NyabsidianRelink`.
